...
export default async function EmployeeAccountArchivedPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold text-slate-900 dark:text-zinc-100">Account archived</h1>
      ...
      <div className="flex flex-wrap gap-3 pt-2">
        {canAccessManagerRoutes(session.user.role as UserRole) ? (
          <Link
            href="/manager"
            className="inline-flex items-center rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-800 dark:text-zinc-200 hover:bg-slate-50"
          >
            Open manager
          </Link>
        ) : null}
